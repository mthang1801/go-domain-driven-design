#!/bin/bash
# Continuous Learning - Session Evaluator (Claude Code Only)
# Runs on Stop hook to extract reusable patterns from Claude Code sessions
#
# Platform: Claude Code only (requires hook system)
# Not compatible with: Cursor, other IDEs
#
...
if [ -f "$CONFIG_FILE" ]; then
  ...
  LEARNED_SKILLS_PATH="$PROJECT_ROOT/$RELATIVE_PATH"
fi
...
session_log="$LEARNED_SKILLS_PATH/session_$timestamp.log"
...
## Patterns Detected
(To be filled by Claude Code evaluation)

## Skills Extracted
(To be filled if patterns are reusable)
EOF

echo "[ContinuousLearning] Session log created: $session_log" >&2
