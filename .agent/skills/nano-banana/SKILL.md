...
## Repo Guidance For Doc Visuals

When using this skill for this repo's documentation visuals:

- start from the article's exact teaching job, not from a style keyword alone
- include `1-3` strong repo-native references when available, especially from `assets/dsa/images`
- include `1-3` external references only to capture idea-space, not to clone composition
- ask for an **explainer image** with a clear reading path, not a generic poster
- if the output looks beautiful but does not teach the right state/boundary/flow, revise the prompt instead of shipping it
